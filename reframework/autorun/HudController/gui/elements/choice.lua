...
function this.draw_hud()
    ...
    if set.combo(gui_util.tr("hud.combo"), "mod.combo_hud", state.combo.hud.values) then
        ...
        hud.request_hud(config_mod.hud[config_mod.combo_hud])
        config.save_global()
    end
    ...
    if imgui.button(gui_util.tr("hud.button_new")) then
        state.input_action = nil
        hud.operations.new()
        config_mod.combo_hud = #config_mod.hud
        hud.request_hud(config_mod.hud[config_mod.combo_hud])
        config.save_global()
    end
    ...
    if imgui.button(gui_util.tr("hud.button_rename")) then
        state.input_action = config_mod.hud[config_mod.combo_hud].name
    end
...
function this.draw_element()
    ...
    if imgui.button(gui_util.tr("hud_element.button_add")) then
        hud.operations.add_element(state.combo.hud_elem:get_key(config_mod.combo_hud_elem) --[[@as string]])
        config.save_global()
    end
