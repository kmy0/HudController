...
function this.hide_nameplate_post(_)
    local name_other = common.get_elem_t("NameOther")
    if name_other and not name_other.hide then
        local enum = e.get("app.cGUIMemberPartsDef.MemberType")
        ...
        if name_other.nameplate_type[enum[type]] then
            return false
        end

        if name_other.pl_draw_distance > 0 and enum[type] == "PL" then
        ...
        elseif name_other.pl_draw_distance > 0 and enum[type] == "SUPPORT_PL" then
            ---@cast GUI020016Part app.GUI020016PartsPlayer
        ...
        elseif name_other.pet_draw_distance > 0 and enum[type] == "SEIKRET" then
        ...
        elseif
            ...
            and (enum[type] == "OT" or enum[type] == "SUPPORT_OT")
        then
