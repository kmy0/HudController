...
function this:new()
    local o = custom_condition.new(self, "Sharpness Changed")
    setmetatable(o, self)
    ...
    o.sharpness = value_checker:new(-1)
...
function this:update()
    ...
    local sharpness_changed = self.sharpness:is_changed(kireaji:get_CurrentValue())
    local frame_ok = self.frame:is_changed(frame_counter.frame)
    if sharpness_changed and frame_ok then
        self.timer:restart()
    end
...
function this:draw_additional_options()
    ...
    changed, options.duration =
        imgui.slider_int("Duration##Sharpness Changed", options.duration, 1, 10)
